...
export const AuthView: React.FC<AuthViewProps> = ({ onLoginSuccess }) => {
    ...
    const handleGuestLogin = () => {
        ...
        onLoginSuccess(guestUser, false); // rememberMe is false for guests
    };

    const handleDevLogin = () => {
        const devUser = authService.loginAsDev();
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="space-y-2">
                    <button type="button" onClick={handleGuestLogin} className="w-full flex justify-center py-3 px-4 border rounded-md shadow-sm text-sm font-bold transition-colors bg-zinc-600 text-slate-200 hover:bg-zinc-500 border-border">
                        المتابعة كزائر
                    </button>
                    <button type="button" onClick={handleDevLogin} className="w-full flex justify-center py-3 px-4 border border-dashed border-zinc-600 rounded-md shadow-sm text-sm font-bold transition-colors bg-zinc-800/50 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200">
                        دخول المطور (مؤقت)
                    </button>
                </div>
